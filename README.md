...
Smart Warehouse API is a RESTful service designed to automate warehouse inventory management. The system allows
businesses to manage product stocks, track item movements between different locations (warehouses), and maintain a
strict history of all transactions (supplies, write-offs, transfers).
...
Businesses often face issues with stock discrepancies ("phantom inventory") or loss of goods. This API solves these
problems by enforcing strict transactional integrity for any change in stock levels. Every change is recorded; nothing
is simply "deleted."
...
4. **Transaction**: An immutable record of stock changes. Types: `IN` (Supply), `OUT` (Sale/Write-off), `TRANSFER` (
...
The application follows the modular architecture provided by NestJS. Each feature is encapsulated in its own module
containing the Controller (API layer), Service (Business Logic), and Repository (Data Access).
...
The database is normalized to ensure data integrity. The central entity is `Inventory` (Stock), linking Products to
Warehouses.
